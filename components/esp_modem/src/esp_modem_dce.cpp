...
{
    ESP_LOGI(TAG, "enter_data");
    if (!device.setup_data_mode()) {
        ESP_LOGI(TAG, "setup failed");
        return false;
    }
    ESP_LOGI(TAG, "device set mode data");
    if (!device.set_mode(modem_mode::DATA_MODE)) {
        ESP_LOGI(TAG, "device set mode failed");
        return false;
    }
    ESP_LOGI(TAG, "dte setmode data");
    if (!dte.set_mode(modem_mode::DATA_MODE)) {
        ESP_LOGI(TAG, "dte set mode failed");
        return false;
    }
...
bool DCE_Mode::set_unsafe(DTE *dte, ModuleIf *device, Netif &netif, modem_mode m)
{
    ...
    case modem_mode::DATA_MODE:
        if (mode == modem_mode::DATA_MODE || mode == modem_mode::CMUX_MODE || mode >= modem_mode::CMUX_MANUAL_MODE) {
            ESP_LOGE(TAG, "Failed to go into data mode");
            return false;
        }
        if (!transitions::enter_data(*dte, *device, netif)) {
            ESP_LOGI(TAG, "Failed to enter data mode");
            return false;
        }
        ESP_LOGI(TAG, "Successfully entered data mode");
        mode = m;
        return true;
