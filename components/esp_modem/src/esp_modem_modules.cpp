...
static const char *TAG = "modem_modules";
...
command_result SIM800::set_data_mode()
{
    ...
    bool NULLMODEM::setup_data_mode()  {
        ESP_LOGE(TAG, "setup_data_mode();");
        return true;
    }

    bool NULLMODEM::set_mode(modem_mode mode) {
        ESP_LOGE(TAG, "set_mode(%d);", (int)mode);
        return true;
    }
