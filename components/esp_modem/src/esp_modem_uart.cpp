...
public:
    explicit UartTerminal(const esp_modem_dte_config *config) :
        ...
        task_handle(config->task_stack_size, config->task_priority, this, s_task) {
            ESP_LOGE(TAG, "Creating UartTerminal");
        }
    ...
    void start() override
    {
        ESP_LOGE(TAG, "Starting UartTerminal");
        signal.set(TASK_START);
    }

    void stop() override
    {
        ESP_LOGE(TAG, "Stopping UartTerminal");
