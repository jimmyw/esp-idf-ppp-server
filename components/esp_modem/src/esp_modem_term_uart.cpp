...
namespace esp_modem {
static const char *TAG = "uart_resource";
    ...
    : port(-1)
{
    ...
    ESP_LOGE(TAG, "Cretign uart resource baud_rate: %d, data_bits: %d parity: %d stop_bits: %d flow_control: %d source_clk: %d",
        config->baud_rate, config->data_bits, config->parity, config->stop_bits, config->flow_control, config->source_clk);
    ...
    if (config->flow_control == ESP_MODEM_FLOW_CONTROL_HW) {
        ESP_LOGI(TAG, "Pins HW_FLOW_CONTROL! port: %d tx: %d rx: %d rts: %d ctx: %d", config->port_num, config->tx_io_num, config->rx_io_num,
